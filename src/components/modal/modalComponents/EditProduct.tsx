...
const EditProduct = () => {
    ...
    const { modalContentType, setModalContentType,setReloadData,itemToEdit } = globalState();
    ...
    return <div>
        <div>
            <div className="w-full bg-gray-100 px-4 py-3 ">
                <p className="flex flex-wrap content-center text-xl">Edit Product</p>
            </div>
            <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                <form className="grid grid-cols-1 md:grid-cols-3 gap-2">
                    <InputText label="Title" name="title" errors={errors} register={register} value={itemToEdit&&itemToEdit['title']}/>
                    <InputSelect label="UOM" name="uom" options={["","BOX","PALLET"]} errors={errors} register={register}/>
                   
                    <InputText label="External Code" name="externalCode" errors={errors} register={register} value={itemToEdit&&itemToEdit['externalCode']}/>
                    <InputText label="Lot No" name="lotNumber" errors={errors} register={register} value={itemToEdit&&itemToEdit['lotNumber']}/>
                    <InputText label="Quantity" name="quantity" errors={errors} register={register} value={itemToEdit&&itemToEdit['quantity']}/>
